...
import cl.uchile.dcc.model.CardClasses.CardAbilities.{BitingFrost, ClearWeather, ImpenetrableFog, NoAbility, TorrentialRain}
import cl.uchile.dcc.model.CardClasses.{MeleeCards, RangeCards, SiegeCards, WeatherCards}
...
class Deck (){
   ...
   * It also sets the size of the deck to 25 and shuffles the cards.
   *
   ...
   */
  def draw_card():Cards={
    // here i don't check if it there is cards to draw because the previous function
   ...
   * Shuffles the deck.
   *
   * This function shuffles the order of the cards in the deck.
   */
  def Shuffle():Unit={
    mazo = Random.shuffle(mazo)
  }
